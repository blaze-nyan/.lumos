...
import { bioengin } from "@/lib/biomedicalengineering";
import Link from "next/link";
import "./chatroom.css";
const prompts = [
  {
    title: "About this Faculty",
    description: "Learn more about the Biomedical.Eng faculty.",
  },
  {
    title: "Schedule",
  ...
  },
  {
    ...
    description:
      "Get information about the subjects in the Biomedical.Eng program.",
  },
  {
    title: "Lecturer Information",
    description:
      "Find out about the lecturers in the Biomedical.Eng department.",
  },
  {
    title: "Further Oppurtunities",
    description: "Oppurtinities after graduating with Biomedical.Eng.",
  },
  {
...
const ChatRoom = () => {
  ...
  useEffect(() => {
    const storedMessages = localStorage.getItem("bmeMessages");
    if (storedMessages) {
      setBmeMessages(JSON.parse(storedMessages));
    }
  }, []);
  useEffect(() => {
    localStorage.setItem("bmeMessages", JSON.stringify(bmeMessages));
  }, [bmeMessages]);
  ...
  const handleSendButtonClick = async (v) => {
    if (v.trim() !== "") {
      ...
      setBmeMessages((prevMessages) => [...prevMessages, newMessage]);
      setShowPrompts(false);
      window.localStorage.setItem("showPrompts", JSON.stringify(showPrompts));
      setInputValue("");

      var response = await bioengin(v);
      if (response == "") {
        response = "Not Available";
      }
      const aiMessage = { text: response, sender: "ai" };
      setBmeMessages((prevMessages) => [...prevMessages, aiMessage]);
  ...
  const handleClearButtonClick = () => {
    setBmeMessages([]);
    localStorage.removeItem("bmeMessages");
    setShowPrompts(true);
    window.localStorage.setItem("showPrompts", JSON.stringify(showPrompts));
  };
    ...
    <div className="chatbot-container-ch">
      ...
      <header className="chatbot-header-ch">
        <h1 className="chatbot-title-ch">
          Lumos for <span>Biomedical.Eng</span>
        </h1>
      </header>
      {showPrompts ? (
