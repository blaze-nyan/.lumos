...
import { bioscience } from "@/lib/biomedicalscience";
import Link from "next/link";
const prompts = [
  {
    title: "About this Faculty",
    description: "Learn more about the Biomed.Sci faculty.",
  },
  {
    title: "Schedule",
  ...
  },
  {
    ...
    description:
      "Get information about the subjects in the Biomed.Sci program.",
  },
  {
    title: "Lecturer Information",
    description: "Find out about the lecturers in the Biomed.Sci department.",
  },
  {
    title: "Further Oppurtunities",
    description: "Oppurtinities after graduating with Biomed.Sci.",
  },
  {
...
const ChatRoom = () => {
  ...
  const [bmsAimessages, setAimessages] = useState([]);

  const [bmsMessages, setBmsMessages] = useState([]);
  const [showPrompts, setShowPrompts] = useState(() =>
    bmsMessages ? false : true
  );

  useEffect(() => {
    const storedMessages = localStorage.getItem("bmsMessages");
    if (storedMessages) {
      setBmsMessages(JSON.parse(storedMessages));
    }
  }, []);
  useEffect(() => {
    localStorage.setItem("bmsMessages", JSON.stringify(bmsMessages));
  }, [bmsMessages]);
  ...
  const handleSendButtonClick = async (v) => {
    if (v.trim() !== "") {
      ...
      setBmsMessages((prevMessages) => [...prevMessages, newMessage]);
      setShowPrompts(false);
      window.localStorage.setItem("showPrompts", JSON.stringify(showPrompts));
      setInputValue("");

      var response = await bioscience(v);
      if (response == "") {
        response = "Not Available";
      }
      const aiMessage = { text: response, sender: "ai" };
      setBmsMessages((prevMessages) => [...prevMessages, aiMessage]);
  ...
  const handleClearButtonClick = () => {
    setBmsMessages([]);
    localStorage.removeItem("bmsMessages");
    setShowPrompts(true);
    window.localStorage.setItem("showPrompts", JSON.stringify(showPrompts));
  };
    ...
    <div className="chatbot-container-ch">
      ...
      <header className="chatbot-header-ch">
        <h1 className="chatbot-title-ch">
          Lumos for <span>Biomed.Sci</span>
        </h1>
      </header>
      {showPrompts ? (
      ...
      <div className="chatbot-content-ch">
        <button onClick={handlePrompts} className="w-full cursor-auto">
          <div className="message-con">
            {bmsMessages.map((message, index) => (
              <div key={index} className={`${messageClass(message?.sender)}`}>
                <div className="message-text">{message?.text}</div>
              </div>
            ))}

            {bmsAimessages.map((message, index) => (
              <div key={index} className={`${messageClass(message?.sender)}`}>
                <div className="message-text">{message?.text}</div>
              </div>
            ))}
