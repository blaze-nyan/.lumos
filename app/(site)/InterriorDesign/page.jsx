...
const prompts = [
  {
    ...
    description: "Learn more about the InteriorDesign faculty.",
  },
  {
    title: "Schedule",
  ...
  },
  {
    ...
    description:
      "Get information about the subjects in the InteriorDesign program.",
  },
  {
    title: "Lecturer Information",
    description:
      "Find out about the lecturers in the InteriorDesign department.",
  },
  {
    title: "Further Oppurtunities",
    description: "Oppurtinities after graduating with InteriorDesign.",
  },
  {
...
const ChatRoom = () => {
  ...
  const [idAimessages, setAimessages] = useState([]);

  const [idMessages, setIdMessages] = useState([]);
  const [showPrompts, setShowPrompts] = useState(() =>
    idMessages ? false : true
  );

  useEffect(() => {
    const storedMessages = localStorage.getItem("idMessages");
    if (storedMessages) {
      setIdMessages(JSON.parse(storedMessages));
    }
  }, []);
  useEffect(() => {
    localStorage.setItem("idMessages", JSON.stringify(idMessages));
  }, [idMessages]);
  ...
  const handleSendButtonClick = async (v) => {
    if (v.trim() !== "") {
      ...
      setIdMessages((prevMessages) => [...prevMessages, newMessage]);
      setShowPrompts(false);
      window.localStorage.setItem("showPrompts", JSON.stringify(showPrompts));
      setInputValue("");

      var response = await intedesign(v);
      if (response == "") {
        response = "Not Available";
      }
      const aiMessage = { text: response, sender: "ai" };
      setIdMessages((prevMessages) => [...prevMessages, aiMessage]);
    ...
    <div className="chatbot-container-ch">
      ...
      <header className="chatbot-header-ch">
        <h1 className="chatbot-title-ch">
          Lumos for <span>Interior Design</span>
        </h1>
      </header>
      {showPrompts ? (
