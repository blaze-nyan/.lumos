...
import { fashion } from "@/lib/fashion";
import Link from "next/link";
const prompts = [
  {
    title: "About this Faculty",
    description: "Learn more about the Fashion faculty.",
  },
  {
    title: "Schedule",
  ...
  },
  {
    ...
    description: "Get information about the subjects in the Fashion program.",
  },
  {
    title: "Lecturer Information",
    description: "Find out about the lecturers in the Fashion department.",
  },
  {
    title: "Further Oppurtunities",
    description: "Oppurtinities after graduating with Fashion.",
  },
  {
...
const ChatRoom = () => {
  ...
  const [faAimessages, setAimessages] = useState([]);

  const [faMessages, setFaMessages] = useState([]);
  const [showPrompts, setShowPrompts] = useState(() =>
    faMessages ? false : true
  );

  useEffect(() => {
    const storedMessages = localStorage.getItem("faMessages");
    if (storedMessages) {
      setFaMessages(JSON.parse(storedMessages));
    }
  }, []);
  useEffect(() => {
    localStorage.setItem("faMessages", JSON.stringify(faMessages));
  }, [faMessages]);
  useEffect(() => {
  ...
  const handleSendButtonClick = async (v) => {
    if (v.trim() !== "") {
      ...
      setFaMessages((prevMessages) => [...prevMessages, newMessage]);
      setShowPrompts(false);
      window.localStorage.setItem("showPrompts", JSON.stringify(showPrompts));
      setInputValue("");

      var response = await fashion(v);
      if (response == "") {
        response = "Not Available";
      }
      const aiMessage = { text: response, sender: "ai" };
      setFaMessages((prevMessages) => [...prevMessages, aiMessage]);
  ...
  const handleClearButtonClick = () => {
    setFaMessages([]);
    localStorage.removeItem("faMessages");
    setShowPrompts(true);
    window.localStorage.setItem("showPrompts", JSON.stringify(showPrompts));
  };
