...
import { hospitality } from "@/lib/hospitality";
import Link from "next/link";
const prompts = [
  {
    title: "About this Faculty",
    description: "Learn more about the Hospitality faculty.",
  },
  {
    title: "Schedule",
  ...
  },
  {
    ...
    description:
      "Get information about the subjects in the Hospitality program.",
  },
  {
    title: "Lecturer Information",
    description: "Find out about the lecturers in the Hospitality department.",
  },
  {
    title: "Further Oppurtunities",
    description: "Oppurtinities after graduating with Hospitality.",
  },
  {
...
const ChatRoom = () => {
  ...
  const [hoAimessages, setAimessages] = useState([]);

  const [hoMessages, setHoMessages] = useState([]);
  const [showPrompts, setShowPrompts] = useState(() =>
    hoMessages ? false : true
  );

  useEffect(() => {
    const storedMessages = localStorage.getItem("hoMessages");
    if (storedMessages) {
      setHoMessages(JSON.parse(storedMessages));
    }
  }, []);
  useEffect(() => {
    localStorage.setItem("hoMessages", JSON.stringify(hoMessages));
  }, [hoMessages]);
  ...
  const handleSendButtonClick = async (v) => {
    if (v.trim() !== "") {
      ...
      setHoMessages((prevMessages) => [...prevMessages, newMessage]);
      setShowPrompts(false);
      window.localStorage.setItem("showPrompts", JSON.stringify(showPrompts));
      setInputValue("");

      var response = await hospitality(v);
      if (response == "") {
        response = "Not Available";
      }
      const aiMessage = { text: response, sender: "ai" };
      setHoMessages((prevMessages) => [...prevMessages, aiMessage]);
  ...
  const handleClearButtonClick = () => {
    setHoMessages([]);
    localStorage.removeItem("hoMessages");
    setShowPrompts(true);
    window.localStorage.setItem("showPrompts", JSON.stringify(showPrompts));
  };
    ...
    <div className="chatbot-container-ch">
      ...
      <header className="chatbot-header-ch">
        <h1 className="chatbot-title-ch">
          Lumos for <span>Hospitality</span>
        </h1>
      </header>
      {showPrompts ? (
