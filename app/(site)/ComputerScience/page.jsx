...
const prompts = [
  {
    ...
    description: "Learn more about the ComputerScience faculty.",
  },
  {
    title: "Schedule",
  ...
  },
  {
    ...
    description:
      "Get information about the subjects in the ComputerScience program.",
  },
  {
    title: "Lecturer Information",
    description:
      "Find out about the lecturers in the ComputerScience department.",
  },
  {
    title: "Further Oppurtunities",
    description: "Oppurtinities after graduating with ComputerScience.",
  },
  {
...
const ChatRoom = () => {
  ...
  const [csAimessages, setAimessages] = useState([]);

  const [csMessages, setCsMessages] = useState([]);
  const [showPrompts, setShowPrompts] = useState(() =>
    csMessages ? false : true
  );

  useEffect(() => {
    const storedMessages = localStorage.getItem("csMessages");
    if (storedMessages) {
      setCsMessages(JSON.parse(storedMessages));
    }
  }, []);
  useEffect(() => {
    localStorage.setItem("csMessages", JSON.stringify(csMessages));
  }, [csMessages]);
  ...
  const handleSendButtonClick = async (v) => {
    if (v.trim() !== "") {
      ...
      setCsMessages((prevMessages) => [...prevMessages, newMessage]);
      setShowPrompts(false);
      window.localStorage.setItem("showPrompts", JSON.stringify(showPrompts));
      setInputValue("");

      var response = await cs(v);
      if (response == "") {
        response = "Not Available";
      }
      const aiMessage = { text: response, sender: "ai" };
      setCsMessages((prevMessages) => [...prevMessages, aiMessage]);
  ...
  const handleClearButtonClick = () => {
    setCsMessages([]);
    localStorage.removeItem("csMessages");
    setShowPrompts(true);
    window.localStorage.setItem("showPrompts", JSON.stringify(showPrompts));
  };
    ...
    <div className="chatbot-container-ch">
      ...
      <header className="chatbot-header-ch">
        <h1 className="chatbot-title-ch">
          Lumos for <span>ComputerScience</span>
        </h1>
      </header>
      {showPrompts ? (
